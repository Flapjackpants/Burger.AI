...
# Minimal safe builtins for guardrail conditions (no open, exec, etc.)
_SAFE_BUILTINS = {"str": str, "len": len, "int": int, "float": float, "bool": bool, "True": True, "False": False, "None": None}
...
class GuardrailEngine:
    ...
    def check_pre_hook(self, tool_name: str, args: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        for rule in self.rules:
            ...
            try:
                ...
                if eval(condition, {"__builtins__": _SAFE_BUILTINS}, safe_locals):
                    return True, rule.get("message", "Action blocked by security guardrail.")
            except Exception as e:
                print(f"[GuardrailEngine] Pre-hook Check Error (skipping rule): {e}")
        return False, None
    ...
    def apply_post_hooks(self, tool_name: str, args: Dict[str, Any], result: Any) -> Any:
        ...
        for rule in self.rules:
            ...
            condition = rule.get("condition", "False")
            try:
                safe_locals = {"args": args, "result": current_result, "tool_name": tool_name}
                if eval(condition, {"__builtins__": _SAFE_BUILTINS}, safe_locals):
                ...
                print(f"[GuardrailEngine] Post-hook Error (skipping rule): {e}")
